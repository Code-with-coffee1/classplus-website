...
exports.createAnnouncement = (req, res) => {
  new_branch.findById(req.body.branchId, (err, branch) => {
      req.body.branchId = branch;
...
exports.getAnnouncementsByBranchIdForAdmin = (req, res) => {
  announcementSchema.find({ "branchId._id": mongoose.Types.ObjectId(req.params.id), admin_announcement: true }, { branchId: 1, admin_announcement: 1, student_announcement: 1, announcement: 1, createdAt: 1 }, { sort: { createdAt: -1 } }, (err, announcement) => {
    if (err) {
      return res.status(401).json({
        error: errorHandler(err),
      });
...
exports.getAnnouncementsByBranchIdForStudent = (req, res) => {
  announcementSchema.find({ "branchId._id": mongoose.Types.ObjectId(req.params.id), student_announcement: true }, { }, { sort: { createdAt: -1 } }, (err, announcement) => {
    if (err) {
      return res.status(401).json({
        error: errorHandler(err),
      });
...
exports.getAnnouncementsForAStudentFromAllBranches = (req, res) => {
  new_branch.find({ students: { $all: [req.query.id] } }, { students: 0 }, function (err, branchData) {
    if (err) {
      ...
      var idArray = branchData.map(function (el) {
        return el._id;
      });

      announcementSchema.find({ "branchId._id": { $in: idArray }, student_announcement: true }, {}, { sort: { createdAt: -1 } }, (err, announcement) => {
        if (err) {
          return res.status(401).json({
            error: errorHandler(err),
          });
...
exports.clearExpiredAnnouncements = (req, res) => {
    announcementSchema.deleteMany({ scheduledTime: { $lt: new Date() } }, (err, result) => {
        if (err) {
            return res.status(401).json({
              error: errorHandler(err),
            });
